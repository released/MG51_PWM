...
unsigned short Motor_Max_PWM_Val = 0ul;
...
void PWM_Set_duty(unsigned short Set_Duty_Percent)
{
    unsigned short Pwm_Duty = 0ul;

    Pwm_Duty = (float) Set_Duty_Percent * Motor_Max_PWM_Val / 1000ul;
    printf("target percent:%d\r\n", Set_Duty_Percent);
    printf("duty:%d\r\n", Pwm_Duty);

    set_SFRS_SFRPAGE;
 
    PWM5H = HIBYTE(Pwm_Duty);
    PWM5L = LOBYTE(Pwm_Duty);
    //  PWM5H = ((unsigned short) Pwm_Duty ) >> 8;
    // PWM5L = ((unsigned short) Pwm_Duty ) & 0xFF;
...
void PWM_Init(void)
{
    unsigned short res = 0ul;
    unsigned short uFrequency = 16000ul;
...
void loop(void)
{
    ...
    static unsigned short pwm_duty = 1000ul;
